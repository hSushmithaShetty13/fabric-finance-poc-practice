...
$platform = @{
  ...
  metadata = @{ type = "DataPipeline"; displayName = $DisplayName; description = "" }
...
Write-Output "Updating pipeline definition for item $ItemId ..."
$result = az rest --method post --resource $resource --url "$api/workspaces/$WorkspaceId/items/$ItemId/updateDefinition`?updateMetadata=true" --headers "Content-Type=application/json" --body "@$bodyFile" 2>&1
